...
{
	var damageDealt = global.damage*(1+global.shotsHit*0.05);
	...
	with (enemy)
	{	
		hp -= global.damage*(1+global.shotsHit*0.05);
	}
	var amountHealed = global.damage*0.8*global.bookOfHealingHealBoost*(1+global.shotsHit*0.05);
	...
	with (obj_player)
	{
		hp += global.damage*0.8*global.bookOfHealingHealBoost*(1+global.shotsHit*0.05);
	}
	global.shotsHit += 1;
	instance_destroy();
}
