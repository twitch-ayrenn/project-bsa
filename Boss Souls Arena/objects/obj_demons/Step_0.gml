...
{
	var damageDealt = 7.5*global.player.conjurationPower;
	...
	with (enemy)
	{	
		hp -= 7.5*global.player.conjurationPower;
	}
	
	with(obj_camera){shake_remain += 4}
	
	var amountHealed = 7.5*global.lifeSteal*global.player.conjurationPower;
	...
	with (obj_allPlayer)
	{
		hp += 7.5*global.lifeSteal*global.player.conjurationPower;
	}
	instance_destroy();
}
