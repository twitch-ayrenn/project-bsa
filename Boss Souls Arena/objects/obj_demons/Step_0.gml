...
{
	var damageDealt = 6.5*global.player.conjurationPower;
	...
	with (enemy)
	{	
		hp -= 6.5*global.player.conjurationPower;
	}
	
	with(obj_camera){shake_remain += 3.5}
	
	var amountHealed = 6.5*global.lifeSteal*global.player.conjurationPower;
	...
	with (obj_allPlayer)
	{
		hp += 6.5*global.lifeSteal*global.player.conjurationPower;
	}
	instance_destroy();
}
