...
{
	//step
	move_towards_point(obj_player.x,obj_player.y-25,generalSpeed);
	if(chooseAnAttack == true)
	{
		chooseAnAttack = false;
		globalvar chooseAttack1; 
		chooseAttack1 = choose(
		...
		if (chooseAttack1 == GeneralAttacks.TpAttack)
		{
			sprite_index = spr_deathKinghts_teleport;
		}
		if (chooseAttack1 == GeneralAttacks.Cone_Attack)
		{
			sprite_index = spr_deathKinghts_cone;
			generalSpeed = 1.4;
		}
		if (chooseAttack1 == GeneralAttacks.Cirkle_Attack)
		{
...
if (phase == 1 && hp <= 0)
{
	...
	if (instance_exists(obj_deathKnightScythe_1)){with(obj_deathKnightScythe_1){instance_destroy();}}
